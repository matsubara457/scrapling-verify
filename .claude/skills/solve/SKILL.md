...
- **依存関係インストール**: プロジェクトタイプに応じて自動実行
  ...
  - Python: `.venv` がなければ `python3 -m venv .venv` で作成 → `.venv/bin/pip install -r requirements.txt`
    - 注意: git worktree には `.venv` はコピーされないため、worktree では毎回 venv 新規作成が必要
...
- 構文チェック: `python3 -c "import py_compile; py_compile.compile('<file>', doraise=True)"` で対象ファイルを検証
...
#### 6f. Python 固有チェック
- `python` ではなく `python3` / `sys.executable` を使っているか？（macOS では `python` が未定義の場合がある）
- `subprocess` でPython を呼び出す場合、`sys.executable` を使用しているか？
- venv 内のパスがハードコードされていないか？
